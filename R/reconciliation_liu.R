#' Forecast reconciliation via Liu-type estimator
#' 
...
#' 
#' @export
reconcile2 <- function(.data, ...){
  UseMethod("reconcile2")
}

#' @rdname reconcile2
#' @export
reconcile2.mdl_df <- function(.data, ...){
  mutate(.data, ...)
}
...
#' 
#' @export
min_trace <- function(models, method = c("wls_var", "ols", "ols-liu", "wls_struct", "mint_cov", "mint_shrink"),
                 sparse = NULL){
  if(is.null(sparse)){
...
forecast.lst_mint_mdl <- function(object, key_data, 
  ...
  } else if(method == "ols-liu"){
    # OLS-Liu
    vliu <- liureg::liu(x, data = as.data.frame(fc), scaling = "centered", d = seq(0, 1, 0.1))
    biasp <- liureg::hatl.liu(vliu)
    W <- diag(biasp[[nrow(covm)]])
  ...
  reconcile2_fbl_list(fc, S, P, W, point_forecast = point_method)
}
...
forecast.lst_btmup_mdl <- function(object, key_data, 
  ...
  reconcile2_fbl_list(fc, S, P, W = diag(nrow(S)),
...
reconcile2_fbl_list <- function(fc, S, P, W, point_forecast, SP = NULL) {
  if(length(unique(map(fc, interval))) > 1){
    abort("Reconciliation of temporal hierarchies is not yet supported.")
  }
