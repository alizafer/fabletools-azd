context("test-reconciliation_liu")

test_that("reconciliation_liu", {
  ...
  fc_agg_reconciled <- fit_agg %>% reconcile2(snaive = min_trace(snaive)) %>% forecast()
  ...
  fc_agg <- fit_agg %>% forecast()
  fc_agg_reconciled <- fit_agg %>% reconcile2(ses = min_trace(ses)) %>% forecast()
  expect_equal(
  ...
  fc_agg_reconciled <- fit_agg %>% reconcile2(ses = min_trace(ses, method = "wls_var")) %>% forecast()
  expect_equal(
    mean(fc_agg_reconciled$value[48 + (1:24)]),
    mean(fc_agg_reconciled$value[(1:24)] + fc_agg_reconciled$value[24 + (1:24)])
  )
  ...
  fc_agg_reconciled <- fit_agg %>% reconcile2(ses = min_trace(ses, method = "ols")) %>% forecast()
  expect_equal(
    mean(fc_agg_reconciled$value[48 + (1:24)]),
    mean(fc_agg_reconciled$value[(1:24)] + fc_agg_reconciled$value[24 + (1:24)])
  )
  ...
  fc_agg_reconciled <- fit_agg %>% reconcile2(ses = min_trace(ses, method = "mint_cov")) %>% forecast()
  expect_equal(
    mean(fc_agg_reconciled$value[48 + (1:24)]),
    mean(fc_agg_reconciled$value[(1:24)] + fc_agg_reconciled$value[24 + (1:24)])
  )
